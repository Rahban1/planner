...
test('implementation prompt requires the proof contract and reserves verification time', () => {
  ...
  assert.match(prompt, /automated UI proof capture command/i)
  assert.match(prompt, /fall back to manual Chromium testing/i)
  assert.match(prompt, /pnpm proof:ui:capture/i)
